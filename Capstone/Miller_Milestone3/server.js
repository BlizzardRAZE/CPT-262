...
app.post("/loginAdmin/", function (req, res) {
  // Declare Variables
  var aUserEMailTM = req.body.userEmailAddressTM;
  var aPassTM = req.body.userPasswordTM;

  // Prepare SQL Command for Select => User Table
  var sqlSelTM =
    "SELECT (dbuseremailaddress) as userEmailAddress, " +
          ...
          bcrypt.compare(
            ...
            function (err, passwordCorrectTM) {
              if (err) {
                ...
                res.send({ redirect: "../admin/insert/insertuser.html" });
              }
            }
          );
        } else {
...
app.post("/newReservation", function (req, res) {
  ...
  // Prepare SQL Command for Insert
  var sqlinsTM =
    "INSERT INTO Reservation(dbuserid, dbplayerid, dbreservationdatetime, dbreservationconfimed) " +
    "VALUES(?, ?, ?, ?)";

  // Place Values Into SQL Command
  var insertsTM = [userNameTM, playNameTM, resDateTime, resConfirmTM];
