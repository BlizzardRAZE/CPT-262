...
var ReservationBox = React.createClass({
  ...
  render: function () {
    // If Player Doesn't have a token, or if there was a token error. Tell player to sign in
    if (this.state.viewThePageTM == 0) {
      return (
        <div className="mainContentBox">
          <div className="titleContainer">
            <h1>Schedule Tee Time</h1>
          </div>

          <div className="directionContainer">
            <div className="directions">
              To Schedule a Tee Time Online you must either login or register an
              account.<br></br>
...
var ReservationInsertForm = React.createClass({
  ...
  loadAllowLogin: function () {
    $.ajax({
      ...
      cache: false,
      success: function (datalog) {
        this.setState({ playerData: datalog });
        this.setState({ playerIdTM: this.state.playerData[0].dbplayerid });
        this.setState({
          playerNameTM:
            this.state.playerData[0].dbplayerfirstname +
            " " +
            this.state.playerData[0].dbplayerlastname,
        });
      }.bind(this),
      error: function (xhr, status, err) {
        // console.error(this.props.url, status, err.toString());
  ...
  getInitialState: function () {
    return {
      // Items pulled from player's session
      playerDataTM: [],
      playerIdTM: 0,
      playerNameTM: "",
      // Only Pass 0 due to Player creating the reservation
      // This way it will mark it as a Online Reservation
      userNameTM: 0,
      numberPlayersTM: "",
      reservationDateTM: "",
      // Only Pass 0 due to Player creating the reservation
      // User will need to approve this reservation
      reservationConfirmTM: 0,
    };
  },
  ...
  handleSubmit: function (e) {
    ...
    // console.log("Player ID: " + this.state.playerIdTM);

    // Get the values from the input fields
    var playerNameTM = this.state.playerIdTM;
  ...
  render: function () {
    return (
      ...
      <form onSubmit={this.handleSubmit} className="formContainer">
        {/* Create Header */}
        <h2> Insert Reservation Details</h2>
        {/* Create Div for form */}
        <div className="formContainer">
          ...
          <table>
            <tbody>
              <tr>
                {/* Show logged in plyaer*/}
                <th>Player's Name</th>
                <td>
                  <TextInput
                  value={this.state.playerNameTM}
                  disable = {true}
                  />
                </td>
              </tr>
