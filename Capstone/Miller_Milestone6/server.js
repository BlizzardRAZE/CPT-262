...
// Login Page for player
app.post("/loginPlayer/", function (req, res) {
  // Declare Variables
  var playEMailTM = req.body.playerEmailAddressTM;
  var playPassTM = req.body.playerPasswordTM;

  // Prepare SQL Command for Select => Player Table
  var sqlSelTM =
    "SELECT (dbplayerid) as playerId, (dbplayeremailaddress) as playerEmailAddress, " +
    "(dbplayerpassword) as playerPassword, (dbrewardid) as playerReward from Player WHERE dbplayeremailaddress = ?";

  // Place Values Into SQL Command
  var insertsTM = [playEMailTM];
  ...
  con.query(sqlTM, function (err, data) {
    ...
    if (data.length > 0) {
      ...
      var playerIdTM = data[0].playerId;
      bcrypt.compare(
        playPassTM,
        ...
        function (err, passwordCorrectTM) {
          if (err) {
            ...
            // Prepare Token to show player
            const token = jwt.sign({ playerIdTM }, jwtKey, {
              algorithm: "HS256",
            ...
            // Redirect Player
            res.send({ redirect: "profile.html" });       
          }
        }
      );
    } else {
...
// Function to run to check if player is logged in to interact with page
app.get("/getPlayerLoggedIn/", function (req, res) {
  // Declare Variables
  var viewPageTM = 0;
  var dataHoldTM = [];
  var payLoadTM;
  ...
  if (!validToken) {
    ...
    try {
      ...
      viewPageTM = payLoadTM.playerIdTM;
...
// Function to remove player's token when logging out
app.get("/getPlayerLoggedOut/", function (req, res) {
          ...
          function (err, passwordCorrectTM) {
            if (err) {
              ...
              if (data[0].userRole === 3) {
                res.send({
                  redirect: "../backend/insert/insertreservation.html",
                });
              }
            }
          }
...
app.post("/updateSinglePlayer", function (req, res) {
  ...
  var playRewardTM = req.body.updatePlayerRewardTM;
